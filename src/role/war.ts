import { calcBodyPart } from "../utils/utils";
...
const roles = {
   ...
   */
  soldier: (data: CreepData): ICreepConfig => ({
    ...battleBase(data.targetFlagName as string, data.keepSpawn as boolean),
    target: creep => {
   ...
   */
  doctor: (data: CreepData): ICreepConfig => ({
    isNeed: () => {
      // data.keepSpawn;
      return false;
    },
   ...
   * @param creepsName 要治疗的 creep 名称
   */
  boostDoctor: (data: CreepData): ICreepConfig => ({
    isNeed: () => false,
    prepare: creep => {
      // 治疗单位不允许发起对穿
      // if (!creep.memory.disableCross) creep.memory.disableCross = true;
   ...
   */
  dismantler: (data: CreepData): ICreepConfig => ({
    ...battleBase(data.targetFlagName as string, data.keepSpawn as boolean),
    target: creep => creep.dismantleFlag(data.targetFlagName as string, data.healerName),
    bodys: "dismantler"
  }),
   ...
   * @param standByFlagName 待命旗帜名称，本角色会优先抵达该旗帜, 直到该旗帜被移除
   */
  boostDismantler: (data: CreepData): ICreepConfig => ({
    ...battleBase(data.targetFlagName as string, data.keepSpawn as boolean),
    ...boostPrepare(),
    target: creep => creep.dismantleFlag(data.targetFlagName as string, data.healerName),
   ...
   * @param flagName 要攻击的旗帜名称
   */
  apocalypse: (data: CreepData): ICreepConfig => {
    // 越界就置为 6
    ...
    return {
      ...
      target: creep => {
        // 获取旗帜
        const targetFlag = creep.getFlag(data.targetFlagName as string);
        if (!targetFlag) {
          creep.say("旗呢?");
          return false;
        }
