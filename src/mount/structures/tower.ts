...
import { ROOM_TRANSFER_TASK, repairSetting } from "../../utils/setting";
import { creepApi } from "../../utils/creepController";
import { whiteListFilter } from "../../utils/utils";
...
export default class TowerExtension extends StructureTower {
  ...
  private defenseWork(): void {
    ...
    this.fire(enemys);
    if (this.room.controller && this.room.controller.checkEnemyThreat()) {
      // 启动主动防御模式
    ...
    for (const log of logs) {
      ...
      // 墙壁被攻击
        ...
        if (target instanceof StructureRampart || target instanceof StructureWall) {
          ...
          const repairerList =
            this.room.controller && this.room.controller.level >= 7 ? [1, 2, 3] : [1, 2, 3, 4, 5, 6, 7, 8];
          // 如果没有维修者的话就进行发布
  ...
  private commandRepair(): boolean {
    ...
    if (!this.room.damagedStructure) {
      ...
      if (damagedStructures.length > 0) {
        const res = this.pos.findClosestByRange(damagedStructures);
        if (res) {
          this.room.damagedStructure = res;
        }
      } else {
  ...
  private commandFillWall(): boolean {
    ...
    // 获取墙壁
    if (!targetWall && focusWall) {
      targetWall = Game.getObjectById(focusWall.id as Id<StructureRampart | StructureWall>);
    }
  ...
  private fire(enemys: (Creep | PowerCreep)[]): ScreepsReturnCode {
    ...
    const target = this.pos.findClosestByRange(enemys);
    if (target) {
