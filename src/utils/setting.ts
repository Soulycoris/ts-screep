...
const getBodyConfig = function (
  ...bodySets: [BodySet, BodySet, BodySet, BodySet, BodySet, BodySet, BodySet, BodySet]
): BodyConfig {
  const config = { 300: [], 550: [], 800: [], 1300: [], 1800: [], 2300: [], 5600: [], 10000: [] } as BodyConfig;
  // 遍历空配置项，用传入的 bodySet 依次生成配置项
  Object.keys(config).map((level, index) => {
    const keys = +level as keyof typeof config;
    config[keys] = calcBodyPart(bodySets[index]);
  });
...
export const labTarget = [
  // 基础
  { target: RESOURCE_HYDROXIDE, count: 500 },
  { target: RESOURCE_ZYNTHIUM_KEANITE, count: 500 },
  { target: RESOURCE_UTRIUM_LEMERGITE, count: 500 },
  // G
  { target: RESOURCE_GHODIUM, count: 5000 },
  // XKHO2 生产线，强化 RANGE_ATTACK
  { target: RESOURCE_KEANIUM_OXIDE, count: 300 },
  { target: RESOURCE_KEANIUM_ALKALIDE, count: 1000 },
  { target: RESOURCE_CATALYZED_KEANIUM_ALKALIDE, count: 4000 },
  // XLHO2 生产线，强化 HEAL
  { target: RESOURCE_LEMERGIUM_OXIDE, count: 300 },
  { target: RESOURCE_LEMERGIUM_ALKALIDE, count: 1000 },
  { target: RESOURCE_CATALYZED_LEMERGIUM_ALKALIDE, count: 4000 },
  // XZHO2 生产线，强化 MOVE
  { target: RESOURCE_ZYNTHIUM_OXIDE, count: 300 },
  { target: RESOURCE_ZYNTHIUM_ALKALIDE, count: 1000 },
  { target: RESOURCE_CATALYZED_ZYNTHIUM_ALKALIDE, count: 4000 },
  // XZH2O 生产线，强化 WORK 的 dismantle
  { target: RESOURCE_ZYNTHIUM_HYDRIDE, count: 300 },
  { target: RESOURCE_ZYNTHIUM_ACID, count: 1000 },
  { target: RESOURCE_CATALYZED_ZYNTHIUM_ACID, count: 4000 },
  // XGHO2 生产线，强化 TOUGH
  { target: RESOURCE_GHODIUM_OXIDE, count: 300 },
  { target: RESOURCE_GHODIUM_ALKALIDE, count: 1000 },
  { target: RESOURCE_CATALYZED_GHODIUM_ALKALIDE, count: 4000 }
];
