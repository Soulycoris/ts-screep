import { DEPOSIT_MAX_COOLDOWN, PB_HARVESTE_STATE } from "../utils/setting";
import { calcBodyPart, getName } from "../utils/utils";
...
const roles: {
   ...
   */
  reiver: (data: CreepData): ICreepConfig => ({
    // 要搬运资源的目标旗帜消失了就不再生成
    isNeed: () => {
      // data.flagName in Game.flags;
      return false;
    },
    // 如果已经统计了移动
    prepare: creep => {
      ...
      //     // 搜索包含存储的目标建筑并存储
      //     let targetStructure: StructureWithStore | Ruin = flag.pos
      //       .lookFor(LOOK_STRUCTURES)
      //       .find(s => "store" in s) as StructureWithStore;
   ...
   */
  claimer: (data: CreepData): ICreepConfig => ({
    // 该 creep 死了不会再次孵化
    isNeed: () => false,
    ...
    target: creep => {
      ...
      //   if (result === OK)
      //     creep.log(
      //       `已清理过期建筑，你可以选择执行 Game.rooms.${creep.room.name}.clearWall() 来清理现存的墙壁`,
      //       "green"
      //     );
   ...
   * 阶段A：预定控制器
   */
  reserver: (data: CreepData): ICreepConfig => ({
    // 该 creep 死了就不会再次孵化
    isNeed: () => false,
    // 向指定房间移动，这里移动是为了避免 target 阶段里 controller 所在的房间没有视野
    prepare: creep => {
   ...
   */
  signer: (data: CreepData): ICreepConfig => ({
    isNeed: () => false,
    target: creep => {
      // if (creep.room.name === data.targetRoomName) {
   ...
   * 如果都造好的话就升级控制器
   */
  remoteBuilder: (data: CreepData): ICreepConfig => ({
    isNeed: () => {
      // const targetRoom = Game.rooms[data.targetRoomName];
   ...
   * 拓展型建造者, 会先抵达指定房间, 然后执行建造者逻辑
   */
  remoteUpgrader: (data: CreepData): ICreepConfig => ({
    isNeed: () => {
      // const targetRoom = Game.rooms[data.targetRoomName];
      // // 如果房间到 6 级了就任务完成
   ...
   * 从指定矿中挖矿 > 将矿转移到建筑中
   */
  remoteHarvester: (data: CreepData): ICreepConfig => ({
    // 如果外矿目前有入侵者就不生成
    isNeed: room => {
      // 旗帜效验, 没有旗帜则不生成
   ...
   */
  depositHarvester: (data: CreepData): ICreepConfig => ({
    isNeed: room => {
      // 旗帜效验, 没有旗帜则不生成
    ...
    source: creep => {
      ...
      //   // 边走边记录抵达时间
      //   if (targetFlag.memory.travelTime == undefined) targetFlag.memory.travelTime = 0;
      //   // 初始化
      //   // 旅途时间还没有计算完成
   ...
   */
  pbAttacker: (data: CreepData): ICreepConfig => ({
    prepare: creep => {
      // const targetFlag = Game.flags[data.sourceFlagName];
      // if (!targetFlag) {
      //   creep.say("旗呢？");
      ...
      //   const powerbank = _.find(
      //     targetFlag.pos.lookFor(LOOK_STRUCTURES),
   ...
   */
  pbHealer: (data: CreepData): ICreepConfig => ({
    target: creep => {
   ...
   */
  pbCarrier: (data: CreepData): ICreepConfig => ({
    // carrier 并不会重复生成
    isNeed: () => false,
    // 移动到目标三格之内就算准备完成
    prepare: creep => {
   ...
   */
  moveTester: (data: CreepData): ICreepConfig => ({
    target: creep => {
      // const targetFlag = Game.flags[data.sourceFlagName];
      // if (!targetFlag) {
