import { ROOM_TRANSFER_TASK, bodyConfigs, creepDefaultMemory, importantRoles } from "../../utils/setting";
  ...
  private getBodys(bodyType: BodyAutoConfigConstant): BodyPartConstant[] {
    ...
    const targetLevel = Object.keys(bodyConfig)
      ...
      .find(level => {
        ...
        const dryCheck =
          this.spawnCreep(bodyConfig[Number(level) as keyof typeof bodyConfig], "bodyTester", { dryRun: true }) === OK;
    ...
    // 获取身体部件
    const bodys: BodyPartConstant[] = bodyConfig[Number(targetLevel) as keyof typeof bodyConfig];
