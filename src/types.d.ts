...
// 房间基础运营
type BaseRoleConstant = "harvester" | "collector" | "miner" | "upgrader" | "filler" | "builder" | "repairer";
 ...
 */
type CreepWork = {
  [role in CreepRoleConstant]: <T extends CreepData>(data: T) => ICreepConfig;
};
/**
 * @prepare isNeed 每次死后都会进行判断，只有返回 true 时才会重新发布孵化任务
 * @prepare prepare 准备阶段执行的方法, 返回 true 时代表准备完成
 * @prepare source creep 获取工作所需资源时执行的方法,返回 true 则执行 target 阶段，返回其他将继续执行该方法
 * @prepare target creep 工作时执行的方法,返回 true 则执行 source 阶段，返回其他将继续执行该方法
 * @prepare bodys 每个角色默认的身体组成部分
 *
 */
...
interface Creep {
  ...
  ids: Id<Creep>;
  move(direction: DirectionConstant | Creep): CreepMoveReturnCode | ERR_NOT_IN_RANGE | ERR_INVALID_TARGET;
  moveEx(direction: DirectionConstant | Creep): CreepMoveReturnCode | ERR_NOT_IN_RANGE | ERR_INVALID_TARGET;
  work(): void;
  checkEnemy(): boolean;
  standBy(): void;
  defense(): void;
  farMoveTo(
    target: RoomPosition,
    range?: number
  ): CreepMoveReturnCode | ERR_NO_PATH | ERR_INVALID_TARGET | ERR_NOT_IN_RANGE;
...
interface Room {
  ...
  hasRunLab: boolean;
  // 该房间是否已经运行过工地作业了
  hasRunConstructionSite: boolean;
  ...
  sources: Source[] | null;
  sourceContainers: StructureContainer[] | null;
  ...
  spawnPbCarrierGroup(flagName: string, num: number): void;
  ...
  getRestrictedPos(): { [creepName: string]: string } | null;
  ...
  // 获取房间中的有效能量来源
  getAvailableSource(): StructureTerminal | StructureStorage | StructureContainer | Source | null;
  ...
  stats: {
    ...
    rooms: {
      [roomName: string]: {
        ...
        [commRes: string]: number | undefined;
      };
    };
  };
...
type BaseLayout = {
  ...
  [controllerLevel in 1 | 2 | 3 | 4 | 5 | 6 | 7 | 8]: {
    // 该类型建筑应该被放置在什么地方
    [structureType in StructureConstant]?: [number, number][] | null | null[];
  };
};
