  ...
  batchRemove(creepNamePart: string): OK | ERR_INVALID_ARGS {
    ...
    Object.keys(Memory.creepConfigs).map(configName => {
      if (configName.includes(creepNamePart)) {
        ...
        if (configName in Game.creeps) {
          Game.creeps[configName].suicide();
        }
      }
    });
  ...
  show(): string {
    ...
    for (const creepName in Memory.creepConfigs) {
      ...
      if (creep) {
        ...
        else liveStats = `${colorful("存活", "green")} 剩余生命 ${Game.creeps[creepName].ticksToLive || "未知"}`;
 ...
 * @param intrval 搜索间隔
 */
export default function creepNumberListener(intrval = 5): void {
